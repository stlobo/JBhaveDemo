...
public class StoryRunner extends JUnitStories {
	...
	public Configuration configuration() {
		...
		return new SeleniumConfiguration().useStoryLoader(
	...
	public InjectableStepsFactory stepsFactory() {
		return new InstanceStepsFactory(configuration(), new WebAppTest());
	}

	/*
	 * This will pick all the feature files by default, if required to run only
	 * a specific one then will have to mention instead of '*'
	 */
	@Override
	protected List<String> storyPaths() {
		return new StoryFinder().findPaths(
				codeLocationFromClass(this.getClass()).getFile(),
				asList("**/GoToGoogle.feature"), null);
	}

	private Format[] formats = new Format[] { CONSOLE, HTML, TXT, XML,
			WEB_DRIVER_HTML };
	...
	private StoryReporterBuilder reporterBuilder = new StoryReporterBuilder()
			...
			.withReporters(new com.generic.utilities.JbehaveReporter())
